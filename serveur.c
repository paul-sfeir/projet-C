...
int extraitRequete(char *requete, size_t longueurRequete, char *nomFichier, size_t maxNomFichier){
    ...
    if(isOk == 1){
        while(requete[i] != ' ' && i<longueurRequete){
            taille++;
            i++;
        }

        if(taille>= maxNomFichier){
            return 0;
        }

        for (i=0; i<taille; i++)
            nomFichier[i] = requete[i+4];
    }
    nomFichier[i] = '\0';
